...
public class SmartDoctorAssignmentService {
        ...
        for (Map.Entry<String, List<String>> entry : keywordMap.entrySet()) {
            if (spec.contains(entry.getKey()) || entry.getKey().contains(spec)) {
                long matches = entry.getValue().stream()
                        ...
                        .count();
                if (matches > 0)
                    return 90.0 + Math.min(matches * 5, 10);
            }
        }
    ...
    private String buildReason(double specScore, double loadScore, double availScore,
        ...
        if (loadScore >= 70)
            reasons.add("low queue load (" + waiting + " waiting)");
        else if (loadScore < 30)
            reasons.add("high queue load (" + waiting + " waiting)");
        if (availScore >= 90)
            reasons.add("currently available");
        else if (availScore < 30)
            reasons.add("limited availability");
        if (historyScore >= 60)
            reasons.add("familiar with patient history");

        if (reasons.isEmpty())
            reasons.add("general recommendation");
