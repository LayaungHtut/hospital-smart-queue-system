...
public class ConsultationDurationService {
    ...
    public double predictDuration(Doctor doctor, int patientAge, String symptoms,
        ...
        double baseMinutes = doctor.getAverageConsultationMinutes() > 0
                ? doctor.getAverageConsultationMinutes() : 15.0;
        ...
        int waitingCount = doctorRepository != null
                ? getDoctorWaitingCount(doctor.getDoctorId()) : 0;
        ...
        log.debug("Duration prediction for Dr. {}: base={} min, final={} min (factors: t={}, d={}, a={}, c={}, t={}, n={}, l={})",
    ...
    public DurationReport getDurationReport(Doctor doctor, int patientAge, String symptoms) {
        ...
        if (avg <= 10) speedCategory = "Fast";
        else if (avg <= 18) speedCategory = "Average";
        else speedCategory = "Thorough";

        String ageGroup;
        if (patientAge < 5) ageGroup = "Infant/Toddler";
        else if (patientAge < 18) ageGroup = "Pediatric";
    ...
    private double getTimeOfDayFactor() {
        ...
        if (hour < 9) return 1.15;       // Early morning, slower start
        if (hour <= 11) return 1.0;       // Morning peak, efficient
        if (hour == 12) return 1.1;       // Lunch, slightly slower
        if (hour <= 14) return 1.05;      // Post-lunch
        if (hour <= 16) return 0.95;      // Afternoon, focused
    ...
    private double getSymptomComplexityFactor(String symptoms) {
        if (symptoms == null || symptoms.isBlank()) return 1.0;

        String lower = symptoms.toLowerCase();
        int complexityScore = 0;

        // Simple symptoms
        if (lower.contains("cold") || lower.contains("cough") || lower.contains("headache")) complexityScore += 1;
        // Moderate symptoms
        if (lower.contains("fever") || lower.contains("pain") || lower.contains("rash")) complexityScore += 1;
        // Complex symptoms
        if (lower.contains("chest") || lower.contains("breathing") || lower.contains("abdomen")) complexityScore += 2;
        if (lower.contains("dizzy") || lower.contains("numbness") || lower.contains("vision")) complexityScore += 2;
        // Multiple symptoms indicator
        String[] commas = symptoms.split("[,;]");
        if (commas.length > 3) complexityScore += 1;

        if (complexityScore <= 1) return 0.85;
        if (complexityScore <= 3) return 1.0;
        return 1.3;
    }

    private double getTypeFactor(String appointmentType) {
        if (appointmentType == null) return 1.0;
