              ...
              <div className="mt-4 rounded-xl border border-primary/20 bg-card overflow-hidden shadow-sm">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    {soapOpen ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                  </div>
                </div>
                  ...
                  <div className="space-y-4 p-4 sm:p-5">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-3">
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="hidden sm:inline">Patient:</span> <strong>{activePatient.patientName}</strong> <span className="hidden sm:inline">({data?.doctor.department})</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {soapNote && (
                          ...
                          >
                            {soapCopied ? <Check className="size-3.5 text-success" /> : <Copy className="size-3.5" />}
                        ...
                        <div className="rounded-lg bg-card p-3 border border-border space-y-2">
                          ...
                          <div className="space-y-1">
                            {soapNote.medications?.map((med, idx) => (
                              <div key={idx} className="text-xs text-foreground bg-accent/40 px-2 py-1 rounded">
                                <strong>{med.name}</strong> {med.dosage} — {med.frequency} ({med.duration})
                                <div className="text-[10px] text-muted-foreground">{med.instructions}</div>
                              </div>
                            ))}
                          </div>
