...
function RegisterPage() {
  ...
  async function submitPersonal(e: React.FormEvent) {
    ...
    if (form.address.trim()) {
      ...
      if (!hasRegion) {
        setError(
          "Address must be within a Myanmar Region/State (e.g. Yangon, Mandalay, Bago, Shan, etc.).",
        );
        return;
      }
      const parts = form.address.split(/[,;\n/]/);
      const hasStreetOrTown =
        lower.includes("st") ||
        lower.includes("road") ||
        lower.includes("lane") ||
        lower.includes("tsp") ||
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            {step === 0 ? (
              <form onSubmit={submitPersonal} className="space-y-5">
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <Field label="Phone Number (Myanmar)">
                    ...
                    <span className="mt-1 block text-[11px] text-muted-foreground">
                      Must start with 09 with 9-11 digits total. Up to 3 family members can share
                      one phone.
                    </span>
                  </Field>
                ...
                {error ? (
                  <p className="rounded-md bg-danger/10 p-2.5 text-sm text-danger">{error}</p>
                ) : null}
